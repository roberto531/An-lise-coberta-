...
<div class="res-header"><div style="font-size:14px;font-weight:700" id="r-nome"></div><div style="font-size:11px;opacity:.85;margin-top:3px" id="r-sub"></div></div>
...
function mascCNPJ(i){
  var v=i.value.replace(/[^0-9]/g,'').slice(0,14);
  if(v.length>=3)v=v.slice(0,2)+'.'+v.slice(2);
  if(v.length>=7)v=v.slice(0,6)+'.'+v.slice(6);
  if(v.length>=11)v=v.slice(0,10)+'/'+v.slice(10);
  if(v.length>=16)v=v.slice(0,15)+'-'+v.slice(15);
  i.value=v;
}
function mascValor(i){
  var v=i.value.replace(/[^0-9]/g,'');
  if(!v){i.value='';return;}
  v=(parseInt(v)/100).toFixed(2);
  i.value=parseFloat(v).toLocaleString('pt-BR',{minimumFractionDigits:2});
}
function selecionarArq(i){
  ...
  r.onload=function(e){
    ...
    z.classList.add('ok');
    z.querySelector('div').textContent='OK';
    var n=document.getElementById('nome-arq');
    n.textContent=f.name;n.style.display='block';
  };
  r.readAsDataURL(f);
}
function addLog(t){
  var c=document.getElementById('log-itens');
  var d=document.createElement('div');
  d.className='log-item';d.textContent=t;c.appendChild(d);
}
...
function analisar(){
  ...
  var cobStr=document.getElementById('cobertura').value.replace(/\./g,'').replace(',','.');
  ...
  .then(function(r){
    addLog('Processando dados do imovel...');
    return r.json();
  })
  .then(function(d){
    if(d.erro){throw new Error(d.erro);}
    addLog('Endereco: '+d.bairro+', '+d.cidade);
    addLog('Area estimada: '+d.area_m2+' m2 - Padrao: '+d.padrao);
...
function renderResultado(d){
  ...
  document.getElementById('r-corpo').innerHTML=
    '<tr><td>Limite atual - '+d.seguradora+'</td><td><strong>'+fmt(d.cobertura_atual)+'</strong></td><td>'+perc+'% '+(exc?'do VR':'coberto')+'</td></tr>'+
    '<tr><td>Valor de risco estimado</td><td>'+fmt(d.valor_risco)+'</td><td>Referencia</td></tr>'+
    '<tr class="'+(exc?'linha-alt':'linha-def')+'"><td>'+(exc?'Excedente':'Diferenca nao coberta')+'</td><td>'+fmt(dif)+'</td><td>'+(exc?'Possivel reducao':'Ajuste necessario')+'</td></tr>'+
  ...
  document.getElementById('r-mem').innerHTML=
    '<div class="mem-linha"><span>CUB/RS R-8N ('+d.mes_cub+')</span><span>'+fmt(d.cub_base)+'/m2</span></div>'+
    '<div class="mem-linha"><span>Fator padrao ('+d.padrao+')</span><span>'+d.fp+'</span></div>'+
    '<div class="mem-linha"><span>Fator localizacao ('+d.bairro+')</span><span>'+d.fl+'</span></div>'+
    '<div class="mem-linha"><span>Area estimada</span><span>'+d.area_m2+' m2</span></div>'+
    '<div class="mem-linha"><span>Custo de construcao</span><span>'+fmt(custo)+'</span></div>'+
...
function gerarPDF(){
  ...
  .then(function(blob){
    ...
    a.href=url;
    a.download='Eleva_Relatorio.pdf';
    a.click();
  })
  .catch(function(e){mostrarErro(e.message);})
  .finally(function(){document.getElementById('btn2').innerHTML='Baixar relatorio PDF';});
}
...
def analisar():
    try:
        ...
        prompt = f"""Analise o PDF da apolice e o CNPJ {cnpj}. Retorne SOMENTE JSON valido sem texto antes ou depois:
{{
...
Extraia da apolice: nome, seguradora, apolice, vigencia, valor de Incendio/Raio/Explosao.
Se cobertura_atual vier zerada, use o valor extraido da apolice.
Estime area pelo numero de andares e padrao construtivo.
CUB/RS R-8N jun/2026 = R$3.150/m2. BDI=18%. Recomendado=arredondar ao milhao acima."""
...
def gerar_pdf():
    try:
        ...
        W,H = A4
        buf = io.BytesIO()
        AZUL=HexColor("#1A4A8B");AZUL_C=HexColor("#EEF3FB");CE=HexColor("#2C2C2C")
        CM=HexColor("#555555");CL=HexColor("#AAAAAA");VM=HexColor("#C0392B")
        LJ=HexColor("#E67E22");CLn=HexColor("#F7F7F7");CB=HexColor("#CCCCCC")
        ...
        meses={"January":"janeiro","February":"fevereiro","March":"marco","April":"abril",
        ...
        c.setFillColor(CL);c.setFont("Helvetica-Oblique",6.5)
        c.drawCentredString(W/2,H-24.5*mm,"Analise de cobertura basica - CUB/RS R-8N SINDUSCON-RS "+d["mes_cub"]+" | Apolice "+d["seguradora"]+" n "+d["apolice"])
        y=H-29*mm
        c.setFillColor(CE);c.setFont("Helvetica-Bold",11.5)
        ...
        y-=4*mm;c.setFont("Helvetica",7.5);c.setFillColor(CM)
        c.drawString(mg,y,"CNPJ: "+d["cnpj_fmt"]+"   |   "+d["seguradora"]+" n "+d["apolice"]+"   |   Vigencia: "+d["vigencia"])
        y-=5*mm;c.setFillColor(AZUL_C)
        c.roundRect(mg,y-6.5*mm,larg,8*mm,2*mm,fill=1,stroke=0)
        c.setFillColor(AZUL);c.setFont("Helvetica-Bold",7.5)
        c.drawString(mg+3*mm,y-3.5*mm,"IMOVEL IDENTIFICADO - "+d["endereco"])
        y-=10*mm
        ...
        t1=Table([
            ...
            ["AREA TOTAL ESTIMADA",str(d["area_m2"])+" m2","Estimativa por tipologia"]
        ...
        c.setFillColor(HexColor("#F9F9F9"));c.setStrokeColor(CB);c.setLineWidth(0.4)
        nh=12*mm;c.rect(mg,y-nh,larg,nh,fill=1,stroke=1)
        c.setFillColor(CM);c.setFont("Helvetica",5.8)
